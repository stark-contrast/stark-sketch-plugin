...
/* harmony default export */ __webpack_exports__["default"] = (function (context) {
  ...
  if (shapeLayers.length + imageLayers.length + textLayers.length === 0) {
    ...
    shapeLayers.forEach(function (layer) {
      if (layer.style.fills) {
        layer.style.fills.forEach(function (fill) {
          var hex = fill.color.substring(0, fill.color.length - 2);
    ...
    webContents.on('did-finish-load', function () {
      webContents.executeJavaScript("setColors('".concat(JSON.stringify(primaryColor), "', '").concat(JSON.stringify(secondaryColor), "', '").concat(hasTextLayer, "')")).catch(console.error);
    });
