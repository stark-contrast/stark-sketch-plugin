...
/* harmony default export */ __webpack_exports__["default"] = (function (context) {
  ...
  if (shapeLayers.length + imageLayers.length + textLayers.length === 0) {
    sketch_ui__WEBPACK_IMPORTED_MODULE_2___default.a.message('Please select two layers to run the contrast checker.');
    ...
    sketch_ui__WEBPACK_IMPORTED_MODULE_2___default.a.message('You can only run the contrast checker against two layers.');
  } else if (imageLayers.length > 0) {
    sketch_ui__WEBPACK_IMPORTED_MODULE_2___default.a.message('You can only check against colors, not gradients or bitmaps.');
  } else {
    ...
    if (primaryColor.a < 255 && secondaryColor.a < 255) {
      sketch_ui__WEBPACK_IMPORTED_MODULE_2___default.a.message('At least one of your layers needs to be at full opacity.');
      return;
    }

    if (primaryColor.a < 255) {
      primaryColor = convertAlpha(secondaryColor, primaryColor);
    }

    if (secondaryColor.a < 255) {
      secondaryColor = convertAlpha(primaryColor, secondaryColor);
    }
    ...
    webContents.on('did-finish-load', function () {
      webContents.executeJavaScript("setColors('".concat(JSON.stringify(colors[0]), "', '").concat(JSON.stringify(colors[1]), "', '").concat(hasTextLayer, "')")).catch(console.error);
    });
